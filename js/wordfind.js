	...
	var WordFind = function () {
		...
		return {
			...
			newPuzzle: function (words, settings) {
				...
				// Filtered letters that are not in the words
				lettersFiltered = letters
					.split("")
				...
				var options = {
					height: wordList[0].length + 2,
					width: wordList[0].length + 12,
			...
			fillBlanks: function (puzzle) {
				for (var i = 0, height = puzzle.length; i < height; i++) {
					...
					for (var j = 0, width = row.length; j < width; j++) {
						if (!puzzle[i][j]) {
							var randomLetter = Math.floor(Math.random() * lettersFiltered.length);
							puzzle[i][j] = lettersFiltered[randomLetter];
						}
					}
				}
			},
