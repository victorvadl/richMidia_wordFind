...
const buttonsFinal = document.getElementById("buttonFinal");
buttonsFinal.classList.add("remove");

// Creates an image object for preloading
var img = new Image();

// Defines the path of the image to be preloaded
var imgUrl = "./img/bg_final.png";

// // Assigns the image path to the image object for preloading
// img.src = imgUrl;
...
(function (document, $, wordfind) {
	...
	var WordFindGame = function () {
		 ...
		 * to the letter that was selected if the game is not completed.
		 *
		 */
		...
		// Function to flash the letters of the hidden words every 5 seconds
		function blinkHiddenWord() {
			setInterval(function () {
				$(".puzzleSquare:not(.wordFound)").each(function () {
					var letter = $(this).text();
					var adjacentSquares = $(this).next().add($(this).prev()); // Seleciona as letras adjacentes
					if (
						adjacentSquares.length === 2 && // Verifica se há exatamente 2 letras adjacentes
						wordList.some(function (word) {
							return word.includes(letter);
						}) &&
						!$(this).hasClass("found") // Verifica se não possui a classe "found"
					) {
		...
		// Calls the function to flash the hidden word
		blinkHiddenWord();
		...
		var endTurn = function () {
			...
			// change background after complete puzzle
			if ($(".puzzleSquare").hasClass("complete")) {
				setTimeout(function () {
					$("#banner").attr("src", imgUrl);
					$(".wordFound").addClass("remove");
					$("#buttonFinal").removeClass("remove");
					buttonFinal.addEventListener("click", function () {
						window.open("https://victorcodes.tech", "_blank");
					});
		...
		return {
			...
			create: function (words, puzzleEl, wordsEl, options) {
				//wordList = words.slice(0).sort(); // original
				...
				$(puzzleEl).on("mouseleave", endTurn); // If mouse move out the puzzle canvas, end turn.
